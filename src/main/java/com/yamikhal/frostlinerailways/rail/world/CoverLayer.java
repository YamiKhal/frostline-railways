 ...
 *
 * Skipped: tunnel rows, stations (with 8 blocks margin), bridge decks and railings, and the footprint of tiled
 * structures. Configured per style (state, chance, reach, biomes), switched off globally with coverLayer.
 */
final class CoverLayer {
    ...
    static void apply(WorldGenLevel level, ChunkPos chunk, RailContext ctx) {
        ...
        List<StructurePlanner.Tile> tiles = StructurePlanner.plan(ctx, level.getServer()).touching(chunk.getMinBlockZ(), chunk.getMaxBlockZ());
        ...
        for (int z = fromZ; z >= toZ; z--) {
            ...
            for (int dx = -layer.reach(); dx <= layer.reach(); dx++) {
                ...
                if (x < chunk.getMinBlockX() || x > chunk.getMaxBlockX() || (bridge && Math.abs(dx) <= deckHalf)
                        || underTile(level, tiles, x, z) || ctx.random(SALT, x, z) >= layer.chance()) {
                    continue;
                }
